...
>(({
  ...
  const [isPopoverVisible, setIsPopoverVisible] = useState(defaultOpen || false);
  ...
  // Base UI has no Radix `asChild`; a custom trigger element goes through the
  // `render` prop instead, and must carry its own content.
  const trigger = React.isValidElement(children) ? (
    <PopoverTrigger ref={ref} {...props} render={children as React.ReactElement} />
  ) : (
    <PopoverTrigger ref={ref} {...props} render={<Button variant="outline" />}>
      {(value || selectedIcon) ? (
        <>
