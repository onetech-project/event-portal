...
*   **Cache:** Redis (single node) — read cache for the event list, ticket_type list, order lists, and the gender master list only, refreshed on every committed write. The gender master list has no write path, so it refreshes on service start instead; it is admitted by name and does not put master data as a category in scope — fee and order-status lists stay out. See Constitution Principle VII.
