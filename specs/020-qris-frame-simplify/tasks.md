...
description: "Task list for 020-qris-frame-simplify"
---

# Tasks: QRIS Frame Simplification

**Input**: Design documents from `/specs/020-qris-frame-simplify/`
...
- [X] T025 [P] Amend `specs/012-manjo-payment-gateway/spec.md`: mark **FR-021a** superseded by spec 020, **FR-021b** retired (with nothing displayed there is no fixed label that can disagree with the code, so the release-time re-verification step and its edge case go), **FR-021c** narrowed to the amount. Update the Clarifications entry, US-scenarios 1 and 4, and withdraw the deferred item "derive the QRIS frame's merchant identity from the code" — there is no longer a displayed identity to derive. Each edit carries a pointer to spec 020.
- [X] T026 [P] Annotate `specs/012-manjo-payment-gateway/research.md`: the frame-identity decision (~L290) and the frame-identity row of the verification table (~L332) are superseded by spec 020. Annotate rather than delete — the record of why the values were once configured, and of the mismatch that release step actually caught, is worth keeping.
