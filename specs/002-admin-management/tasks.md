...
- [x] T001 Add JWT library (`golang-jwt/jwt`) and password hashing library
      (bcrypt or argon2id) to `backend/go.mod`
- [x] T002 [P] Add admin seed script/SQL (`backend/migrations/seed_admin.sql` or a
...
- [x] T003 Define the `OrderChecker` interface in
...
- [x] T004 Implement `OrderChecker` against the `order` domain's own tables in
      ...
      no N+1)
- [x] T005 Inject the `order` domain's `OrderChecker` implementation into the
      `event` service in `backend/cmd/api/main.go` (depends on T004)
- [x] T006 [P] Add `admin` domain DTOs (`LoginRequest`, `LoginResponse`) in
      `backend/internal/admin/dto.go`
- [x] T007 Implement JWT issuance/verification helpers in
      `backend/internal/admin/jwt.go`
- [x] T008 Implement JWT auth Echo middleware in
...
- [x] T009 [P] [US1] Implement `admin` repository lookup-by-email query in
      `backend/internal/admin/repository.go`
- [x] T010 [US1] Implement `admin` service `Login(email, password)` (bcrypt/argon2
      verify + JWT issuance via T007) in `backend/internal/admin/service.go`
      (depends on T007, T009)
- [x] T011 [US1] Implement `POST /api/v1/admin/login` handler in
      `backend/internal/admin/handler.go`, registered in `backend/cmd/api/main.go`
- [x] T012 [P] [US1] Build `frontend/app/admin/login/page.tsx` (login form, stores
...
- [x] T013 [P] [US2] Add admin-facing `event` DTOs (`EventAdminView`,
...
- [x] T014 [P] [US2] Implement admin `event` repository CRUD queries (all
      statuses, not just PUBLISHED) in `backend/internal/event/repository.go`
- [x] T015 [US2] Implement the transactional event delete in
...
- [x] T016 [US2] Implement `event` service create/update validation (slug
      uniqueness error mapping, `end_date >= start_date`) in
      `backend/internal/event/service.go` (depends on T013, T014)
- [x] T017 [US2] Implement `event` service `DeleteEvent` mapping the guard outcome
      ...
      T015)
- [x] T018 [US2] Implement `CRUD /api/v1/admin/events` handlers (list, create, get,
...
- [x] T019 [P] [US2] Build `frontend/app/admin/events/page.tsx` (list + create
      form; banner is a plain URL text input, not a file picker)
- [x] T020 [P] [US2] Build `frontend/app/admin/events/[id]/page.tsx` (edit event
...
- [x] T021 [P] [US3] Add admin-facing `TicketTypeAdminView`,
...
- [x] T022 [P] [US3] Implement admin ticket-type repository CRUD queries in
...
- [x] T023 [US3] Implement the transactional guarded delete for ticket types in
...
- [x] T024 [US3] Implement ticket-type service create/update validation
      ...
      on T021, T022)
- [x] T025 [US3] Enrich ticket-type list/detail responses with the derived
...
- [x] T026 [US3] Implement ticket-type service `DeleteTicketType` mapping
      zero-rows-deleted to 404 vs 400 `TICKET_TYPE_HAS_ORDERS` in
      `backend/internal/event/service.go` (depends on T023)
- [x] T027 [US3] Implement the flat ticket-type handlers in
...
- [x] T028 [P] [US3] Build ticket-type management UI within
...
- [x] T029 [P] [US4] Add `OrderSummary` and `AttendeeSummary` DTOs in
      `backend/internal/order/dto.go` (no sqlc struct leaks, Constitution
      Principle III)
- [x] T030 [US4] Implement read-only order/attendee list queries (with optional
...
- [x] T031 [US4] Implement `GET /api/v1/admin/orders` and
...
- [x] T032 [P] [US4] Build `frontend/app/admin/orders/page.tsx` (read-only order
      list)
- [x] T033 [P] [US4] Build `frontend/app/admin/attendees/page.tsx` (read-only
      attendee list)
...
- [x] T034 [P] Add `frontend/components/admin/admin-nav.tsx` shared navigation
      across all `/admin/*` pages
- [x] T035 [P] Add structured error codes (`SLUG_NOT_UNIQUE`, `INVALID_DATE_RANGE`,
      `EVENT_HAS_ORDERS`, `TICKET_TYPE_HAS_ORDERS`) consistently in
      `backend/internal/event/handler.go`
- [x] T036 Run `quickstart.md` end-to-end (login → create event/ticket type via the
      flat routes → verify remaining-quota/sold semantics → attempt blocked deletes
      → view orders/attendees) and fix discrepancies
- [x] T037 [P] Review `internal/admin`, `internal/event`, and `internal/order`
      `dto.go` files to confirm no sqlc-generated struct leaks into HTTP responses
      (Constitution Principle III)
- [x] T038 [P] Verify Domain Isolation (Constitution Principle II /
