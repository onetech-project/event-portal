...
Quota Re-deduction (Settle after expiry, spec 012): The one webhook outcome that takes quota rather than restoring it. A completed-payment notification for an order this system itself expired MUST settle it — the gateway can be asked to redeliver a notification, so a callback lost to an outage is recovered by replaying it rather than by a person recording the payment by hand. Settling re-deducts every ticket type the order holds inside the same transaction as the status change, all-or-nothing: if any type is short, nothing moves, the order stays EXPIRED, and the handler still answers 200 with a body naming the shortfall (a non-200 would burn the gateway's retry budget on an attempt guaranteed to fail identically). Because that 200 shares its status line with every ordinary acknowledgement, the refusal is carried by the envelope's numeric code — `200001` / `TICKETS_UNAVAILABLE` against `200000` — and not by the status; anything asserting on this endpoint must read the code. An order the *gateway* rejected or cancelled MUST NOT be settled this way — only this system's own expiry verdict is reversible. **No order that has a payment session may reach PAID by any path other than a gateway webhook** — that rule is absolute and unchanged.

Free Registration (spec 022): the one write path that reaches PAID without a gateway. `POST /api/v1/ticket/register/:ticket_type_id` wraps the creation of a zero-total `orders` row at PAID, one `order_items` line, one fully-filled `attendees` row, and the atomic quota deduction in a single transaction — the same shape booking uses, under the same no-network-call and no-cache-call rules. It opens no payment session, calls no gateway, writes no `payments` row and no `order_fees`, and sets `payment_expires_at` NULL so no sweeper and no countdown ever sees it. Nothing marks the order as a registration: the two origins of PAID are kept distinguishable by DERIVATION (Principle IV v6.0.0) — an order is registration-originated exactly when it carries a line for a ticket type with `is_visible = FALSE`, which containment guarantees can only have come from this path. Any surface reporting revenue applies that derivation and never reads the status alone. The accepted cost is that the value is not stable over time: making an invitation ticket type purchasable again retroactively reclassifies every historical order that used it. Ticket issuance and email delivery then run off the request path in a goroutine mirroring the webhook's `fulfillAsync`, and the delivery carries one attachment (the E-Ticket) with no receipt and no monetary figure.

Terms agreement staleness (spec 022): `event_terms.event_id` is UNIQUE and `UpsertEventTerms` is `ON CONFLICT ... DO UPDATE`, so an edit **overwrites in place and preserves the row id**. The version token is therefore `event_terms.updated_at`, never the id — a check comparing ids can never detect an edit. This governs both the booking agreement and the registration submit, so agreeing means one thing on both surfaces.
